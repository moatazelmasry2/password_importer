...
# Ensure PGDATA exists on the NAS and is mode 700 (from macOS)
...
docker run -d \
  --name combos \
  -p 5432:5432 \
  -v /Volumes/nfs/combos_pgdata:/var/lib/postgresql \
  -e PGDATA=/var/lib/postgresql/pgroot \
...
PGDATA="${PGDATA:-/var/lib/postgresql/pgroot}"
PG_BIN="/usr/lib/postgresql/17/bin"

mkdir -p "$PGDATA"
chmod 700 "$PGDATA" || true

# Detect the real owner of PGDATA as seen INSIDE the container
owner_uid=$(stat -c "%u" "$PGDATA")
owner_gid=$(stat -c "%g" "$PGDATA")

echo "PGDATA owner detected: ${owner_uid}:${owner_gid}"

# Create matching group/user inside the container if needed
if ! getent group "${owner_gid}" >/dev/null 2>&1; then
  groupadd -g "${owner_gid}" pggrp || true
fi
if ! id -u "${owner_uid}" >/dev/null 2>&1; then
  useradd -u "${owner_uid}" -g "${owner_gid}" -M -d /var/lib/postgresql -s /bin/bash pgusr || true
fi

# Initialize once (using a pw file owned by that uid)
if [ ! -f "$PGDATA/PG_VERSION" ]; then
  pwfile="/tmp/pgpass.$$"
  # create as the detected uid:gid so initdb can read it
  gosu "${owner_uid}:${owner_gid}" sh -c "umask 177; echo \"$POSTGRES_PASSWORD\" > $pwfile"

  gosu "${owner_uid}:${owner_gid}" "$PG_BIN/initdb" -D "$PGDATA" \
    --username="${POSTGRES_USER:-postgres}" \
    --pwfile="$pwfile"

  gosu "${owner_uid}:${owner_gid}" rm -f "$pwfile"

  # Allow localhost + Docker host gateway (192.168.65.1) to connect
...
# Start postgres (listening on all addrs) with your tuning flags
